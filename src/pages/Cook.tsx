      ...
      <div className="px-5 py-6 lg:px-12 lg:py-10 max-w-4xl mx-auto">
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
          <motion.div variants={fadeUp} className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-display font-bold text-foreground flex-1 flex items-center gap-2"><UtensilsCrossed size={24} /> What Can I Cook?</h1>
          ...
          <div className="space-y-4 lg:grid lg:grid-cols-2 lg:gap-5 lg:space-y-0">
            {recipes.map((r, i) => (
              <motion.div
                key={i}
                variants={fadeUp}
                className={`glass-card space-y-3 p-5 lg:p-6 ${r.featured ? "border-primary/20 bg-gradient-to-br from-primary/[0.04] to-teal-400/[0.03] lg:col-span-2" : ""}`}
              >
                {r.featured && <span className="text-sm font-bold text-primary flex items-center gap-1.5"><Sparkles size={16} /> RECOMMENDED</span>}
                ...
                <div className="flex items-center gap-3 text-sm text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1"><Flame size={14} /> {r.cal} cal</span>
                </div>
                <div className="flex items-center gap-3 text-sm text-muted-foreground">
                  <span className="flex items-center gap-1"><Dumbbell size={14} /> {r.protein}g protein</span>
                ...
                {r.warning && <p className="text-sm text-accent font-medium flex items-center gap-1.5"><AlertTriangle size={14} /> {r.warning}</p>}
                {r.badge && <span className="chip text-xs py-1.5 flex items-center gap-1 w-fit"><Zap size={12} /> {r.badge}</span>}
                ...
                <div className="flex gap-3 pt-1">
                  ...
                  {cooked === i ? (
                    ...
                    <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleCook(i, r.name)} className="btn-primary text-sm py-2 px-5 flex items-center gap-1.5"><UtensilsCrossed size={14} /> Cook This!</motion.button>
                  )}
                </div>
                      ...
                      <div className="pt-3 border-t border-white/[0.05] space-y-2.5">
                        {r.steps.map((s, j) => (
                          <div key={j} className="flex gap-3 text-sm">
                            <span className="w-6 h-6 rounded-full bg-primary/15 text-primary flex items-center justify-center shrink-0 text-xs font-bold">{j + 1}</span>
          ...
          <motion.div variants={fadeUp} className="glass-card-static bg-primary/[0.03] flex items-start gap-4 p-5">
