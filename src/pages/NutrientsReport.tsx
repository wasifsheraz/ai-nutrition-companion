...
function StatCard({ icon: Icon, label, value, sub, trend }: {
  ...
  return (
    <div className="glass-card-static p-4 lg:p-5 space-y-2">
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6 lg:space-y-8">
          ...
          <motion.div variants={fadeUp} className="glass-card-static p-6 space-y-4">
            ...
            <div className="flex items-end gap-2 h-40 relative pt-6">
              {/* Target line */}
              <div className="absolute left-0 right-0 border-t border-dashed border-primary/25"
                style={{ bottom: `${(targetCal / maxCal) * 100}%` }}>
                <span className="text-[10px] text-primary/60 absolute -top-3.5 right-0">{targetCal}</span>
              </div>
              {dailyCals.map((d, i) => {
          ...
          <motion.div variants={fadeUp} className="glass-card-static p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {macros.map((m, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-4 p-3 rounded-2xl" style={{ background: "rgba(255,255,255,0.02)" }}>
          ...
          <motion.div variants={fadeUp} className="flex gap-3">
            ...
            <motion.button whileTap={{ scale: 0.97 }} onClick={() => navigate("/meal-plan")}
