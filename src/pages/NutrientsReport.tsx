...
import { ChevronLeft, ChevronRight, BrainCircuit, Share2, Target, Trophy, AlertTriangle, Flame, Star, Dumbbell, TrendingUp, CalendarDays, CheckCircle2, XCircle } from "lucide-react";
...
const highlights = [
  { icon: Trophy, text: "Best Day: Wednesday", color: "border-primary/20" },
  { icon: AlertTriangle, text: "Worst Day: Saturday", color: "border-accent/20" },
  { icon: Flame, text: "Most Eaten: Chicken (4×)", color: "border-teal-400/20" },
  { icon: Star, text: "Top Rated: Palak Paneer", color: "border-amber-400/20" },
];
...
function GradeReveal() {
  ...
  return (
    <div className="glass-card-static text-center py-10 lg:py-14 space-y-4 border-primary/20 relative overflow-hidden">
      ...
      >
        <span className="text-8xl lg:text-9xl font-display font-bold gradient-text inline-block" style={{ filter: "drop-shadow(0 0 30px rgba(16,185,129,0.4))" }}>B+</span>
      </motion.div>
      ...
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} className="text-sm text-muted-foreground relative z-10 flex items-center justify-center gap-1.5">
        <TrendingUp size={16} className="text-primary" /> Up from B last week
      </motion.p>
    </div>
  );
}
      ...
      <div className="px-5 py-6 lg:px-12 lg:py-10 max-w-5xl mx-auto">
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6 lg:space-y-8">
          <motion.div variants={fadeUp} className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-display font-bold text-foreground flex items-center gap-2"><BarChart3 size={24} /> Weekly Report</h1>
          </motion.div>
          ...
          <div className="lg:grid lg:grid-cols-2 lg:gap-6 space-y-5 lg:space-y-0">
            ...
            <motion.div variants={fadeUp} className="glass-card-static space-y-4 p-6">
              <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider flex items-center gap-2"><TrendingUp size={16} /> Calorie Tracking</h3>
              ...
              <div className="flex items-end gap-2 h-36 relative pt-4">
                <div className="absolute left-0 right-0 border-t border-dashed border-primary/30" style={{ bottom: `${(1800 / maxCal) * 100}%` }}>
                  <span className="text-[10px] text-primary/60 absolute -top-3.5 right-0">1,800</span>
                </div>
                {dailyCals.map((d, i) => (
                  <motion.div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                    <motion.div
                      className={`w-full rounded-lg ${d.ok ? "bg-gradient-to-t from-primary/80 to-primary" : "bg-gradient-to-t from-accent/80 to-accent"}`}
                      initial={{ height: 0 }}
            ...
            {/* Macros */}
            <motion.div variants={fadeUp} className="glass-card-static space-y-4 p-6">
              <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider flex items-center gap-2"><Dumbbell size={16} /> Macro Balance</h3>
              {macros.map((m, i) => (
          ...
          {/* Meal Consistency */}
          <motion.div variants={fadeUp} className="glass-card-static space-y-4 p-6">
            <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider flex items-center gap-2"><CalendarDays size={16} /> Consistency: 19/21 (90%)</h3>
                  ...
                  <div className="space-y-1">
                    {mealGrid[i].map((m, j) => (
                      <motion.div
                        ...
                        className={`w-full h-4 rounded flex items-center justify-center ${m ? "bg-primary/50" : "bg-destructive/30"}`}
                      >
            ...
            <p className="text-sm text-primary font-semibold flex items-center gap-1.5"><Flame size={16} /> Best streak: 5 days straight!</p>
          </motion.div>
            ...
            {highlights.map((h, i) => (
              ...
              >
                <div className="w-10 h-10 rounded-xl bg-white/[0.04] flex items-center justify-center mx-auto">
                  <h.icon size={20} className="text-muted-foreground" />
                </div>
          ...
          {/* AI Insights */}
          <motion.div variants={fadeUp} className="glass-card-static bg-primary/[0.03] space-y-3 p-6">
          ...
          <motion.div variants={fadeUp} className="flex gap-3">
            ...
            <motion.button whileTap={{ scale: 0.97 }} onClick={() => navigate("/meal-plan")} className="btn-primary flex-1 py-3.5 text-sm font-bold flex items-center justify-center gap-2">
              <Target size={18} /> Start Next Week
            </motion.button>
          </motion.div>
        </motion.div>
      </div>
