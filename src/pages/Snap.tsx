...
export default function Snap() {
  ...
  return (
    <AppLayout>
      <div className="px-5 py-6 lg:px-12 lg:py-10 max-w-3xl mx-auto relative">
        {/* Background blob */}
        <motion.div
          animate={{ opacity: [0.01, 0.03, 0.01] }}
          transition={{ duration: 8, repeat: Infinity }}
          className="absolute top-[10%] right-[-100px] w-[400px] h-[400px] bg-primary rounded-full blur-[120px] pointer-events-none"
        />
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6 relative z-10">
          ...
          <AnimatePresence mode="wait">
            {phase === "upload" && (
              <motion.div key="upload" initial={{ opacity: 0, filter: "blur(6px)" }} animate={{ opacity: 1, filter: "blur(0px)" }} exit={{ opacity: 0, filter: "blur(6px)" }} className="space-y-6">
                <motion.div
                  variants={fadeUp}
                  whileHover={{ y: -3 }}
                  whileTap={{ scale: 0.98 }}
                  className="glass-card-static border-2 border-dashed border-primary/12 flex flex-col items-center justify-center py-20 cursor-pointer active:bg-primary/[0.02] transition-colors"
                  onClick={startScan}
                >
                  <motion.div
                    animate={{ y: [0, -6, 0] }}
                    transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                    className="icon-box-lg mb-5"
                  >
                ...
                <div className="flex gap-3">
                  <motion.button whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.97 }} onClick={startScan} className="btn-primary flex-1 py-4 text-base font-semibold flex items-center justify-center gap-2">
                    <Camera size={20} strokeWidth={1.5} /> Take Photo
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.97 }} onClick={startScan} className="btn-secondary flex-1 py-4 text-base font-medium flex items-center justify-center gap-2">
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    {[
                      ...
                      <motion.button key={i} whileHover={{ y: -3 }} whileTap={{ scale: 0.95 }} onClick={startScan} className="glass-card text-center py-4 px-4 text-sm flex-1 flex flex-col items-center gap-2">
            ...
            {phase === "loading" && (
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8 text-center py-16">
                <div className="relative w-48 h-48 mx-auto rounded-2xl overflow-hidden glass-card-static flex items-center justify-center">
                  <ScanLine size={64} className="text-muted-foreground/15" strokeWidth={1} />
                  <motion.div
            ...
            {phase === "results" && (
              <motion.div key="results" initial={{ opacity: 0, y: 16, filter: "blur(6px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} className="space-y-5">
                <div className="glass-card-static space-y-5 p-6 lg:p-8">
                  <div className="flex items-center justify-between">
                    ...
                    <motion.div
                      ...
                      className="w-14 h-14 rounded-full bg-primary/8 flex items-center justify-center text-base font-bold text-primary border border-primary/12"
                    >
                      7/10
                    </motion.div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {[
                      ...
                      <motion.div
                        ...
                        className="glass-card-static text-center py-4"
                      >
                ...
                <div className="glass-card-static space-y-2 p-5">
                  ...
                  <div className="h-3.5 rounded-full bg-muted/20 overflow-hidden flex">
                ...
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
                  className="glass-card-static bg-primary/[0.02] border-primary/8 text-sm text-foreground flex items-center gap-3 p-4">
                ...
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}
                  className="glass-card-static bg-accent/[0.02] border-accent/8 text-sm text-foreground flex items-center gap-3 p-4">
                ...
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}
                  className="glass-card-static bg-primary/[0.02] text-sm p-4 flex items-center gap-3">
                ...
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }}
                  className="glass-card-static text-sm text-muted-foreground p-4 flex items-center gap-3">
                ...
                <div className="flex gap-3 sticky bottom-[96px] lg:bottom-4 pt-3">
                  {!added ? (
                    <motion.button whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.97 }} onClick={handleAdd} className="btn-primary flex-1 py-4 text-base font-bold flex items-center justify-center gap-2">
