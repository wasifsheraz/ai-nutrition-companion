...
import {
  ...
  ChevronRight, Flame, Trophy } from "lucide-react";
...
function FloatingBlob({ className, delay = 0 }: { className: string; delay?: number }) {
  return (
    <motion.div
      animate={{ opacity: [0.01, 0.035, 0.01], scale: [1, 1.15, 1] }}
...
export default function Onboarding() {
  ...
  if (showResult) {
    return (
      <div className="min-h-[100dvh] flex items-center justify-center px-5 py-8 relative overflow-hidden">
        <FloatingBlob className="w-[500px] h-[500px] bg-primary top-[-100px] right-[-100px]" />
        <FloatingBlob className="w-[400px] h-[400px] bg-secondary bottom-[-50px] left-[-50px]" delay={3} />
        ...
        >
          ...
          >
            ...
            >
              <div className="icon-box-lg mx-auto" style={{ background: "rgba(16,185,129,0.1)", borderColor: "rgba(16,185,129,0.2)" }}>
          ...
          <motion.div
            ...
            className="glass-card-static p-4 space-y-2 border-primary/10"
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col px-5 py-6 lg:py-8 max-w-lg mx-auto relative">
      <FloatingBlob className="w-[400px] h-[400px] bg-primary top-[10%] right-[-150px]" />
      <FloatingBlob className="w-[300px] h-[300px] bg-secondary bottom-[20%] left-[-100px]" delay={5} />
            ...
            return (
              <div key={i} className="flex items-center">
                <motion.div
                  animate={{
                    scale: active ? 1.1 : 1,
                    borderColor: done || active ? "hsl(160,84%,39%)" : "rgba(255,255,255,0.05)",
                  }}
                  ...
                  className={`w-10 h-10 lg:w-11 lg:h-11 rounded-xl flex items-center justify-center border-2 transition-colors ${
                    ...
                    active ? "bg-primary/8 border-primary" :
                ...
                {i < steps.length - 1 && (
                  <motion.div
                    animate={{ backgroundColor: i < step ? "hsl(160,84%,39%)" : "rgba(255,255,255,0.03)" }}
                    className="w-6 lg:w-10 h-0.5 mx-1 rounded-full"
                  />
                )}
              </div>
            );
            ...
            <motion.div key="s0" {...fadeSlide} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 justify-center">
                    <motion.button whileTap={{ scale: 0.85 }} onClick={() => setAge(Math.max(10, age - 1))}
                      className="w-11 h-11 rounded-xl bg-muted/30 border border-border/30 flex items-center justify-center text-lg text-foreground font-bold hover:bg-muted/50 transition-colors">
                      −
                    </motion.button>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2.5">
                  {genderOptions.map((g) => (
                    <motion.button key={g.label} whileHover={{ y: -2 }} whileTap={{ scale: 0.95 }} onClick={() => setGender(g.label)}
                      ...
                      {gender === g.label && (
                        <motion.div layoutId="gender-glow" className="absolute inset-0 bg-gradient-to-br from-primary/8 to-transparent" />
                      )}
                      <div className="relative z-10">
                        <div className="w-8 h-8 rounded-lg bg-muted/30 border border-border/30 flex items-center justify-center mx-auto mb-1.5">
                        ...
                        {gender === g.label && (
                          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 500 }}
                            className="w-4 h-4 bg-primary rounded-full flex items-center justify-center mx-auto mt-1">
            ...
            <motion.div key="s1" {...fadeSlide} className="space-y-6">
              ...
              <motion.div layout className="glass-card-static p-6 text-center relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-3">
                  <div className="icon-box mx-auto" style={{ background: "rgba(16,185,129,0.08)", borderColor: "rgba(16,185,129,0.15)" }}>
                  ...
                  <motion.p key={bmi.toFixed(1)} initial={{ scale: 1.3, opacity: 0, filter: "blur(4px)" }} animate={{ scale: 1, opacity: 1, filter: "blur(0px)" }}
                    className={`text-6xl font-bold font-display ${bmiCategory.color}`}
                    style={{ filter: bmiCategory.color === "text-primary" ? "drop-shadow(0 0 20px rgba(16,185,129,0.25))" : undefined }}>
                    {bmi.toFixed(1)}
                  </motion.p>
          ...
          {step === 2 && (
            <motion.div key="s2" {...fadeSlide} className="space-y-6">
                ...
                <div className="space-y-3">
                  {activityLevels.map((a, i) => (
                    <motion.button
                      ...
                      className={`glass-card w-full flex items-center gap-3 py-3 px-4 text-left transition-all relative overflow-hidden ${activity === a.label ? "border-primary/25 bg-primary/[0.04]" : ""}`}
                    >
                      ...
                      <div className="icon-box-sm relative z-10"
                        style={activity === a.label ? { background: "rgba(16,185,129,0.1)", borderColor: "rgba(16,185,129,0.15)" } : {}}>
                      ...
                      {activity === a.label && (
                        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 500 }}
                          className="w-6 h-6 bg-primary rounded-full flex items-center justify-center relative z-10">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2.5">
                  {healthGoals.map((g) => (
                    <motion.button key={g.label} whileHover={{ y: -2 }} whileTap={{ scale: 0.95 }} onClick={() => setGoal(g.label)}
                      className={`glass-card text-center py-3.5 px-2 transition-all relative overflow-hidden ${goal === g.label ? "border-primary/25 bg-primary/[0.04]" : ""}`}>
                      {goal === g.label && <motion.div layoutId="goal-glow" className="absolute inset-0 bg-gradient-to-br from-primary/8 to-transparent" />}
                      <div className="relative z-10">
                        <div className="icon-box-sm mx-auto mb-2" style={goal === g.label ? { background: "rgba(16,185,129,0.1)", borderColor: "rgba(16,185,129,0.15)" } : {}}>
                ...
                {cuisines.map((c, i) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`glass-card text-center py-3.5 px-2 transition-all relative overflow-hidden ${selected ? "border-primary/25 bg-primary/[0.04]" : ""}`}
                    >
                      ...
                      <div className="relative z-10">
                        <div className="icon-box-sm mx-auto mb-2" style={selected ? { background: "rgba(16,185,129,0.1)", borderColor: "rgba(16,185,129,0.15)" } : {}}>
      ...
      {/* ── Navigation ── */}
      <div className="flex gap-3 mt-8 pb-safe relative z-10">
        {step > 0 && (
          <motion.button
